...
class NWCollectd:
    ...
    def init(self):
        ...
        try:
            self.ns.publishService(self.cluster+".allpoints", "tcp://%s:%s" %
                                   (self.ip, port), self.publishTimeout, "pub/sub", "Point")
    ...
    def write(self, vl, data=None):
        for (i, val) in enumerate(vl.values):
            ...
            convertedValue = self.convertValue(
                vl.host, name, val, vl.time, dstype)
