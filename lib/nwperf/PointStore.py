...
class PointStore(object):
    ...
    def loadHosts(self):
        try:
            self.index2host = [i.rstrip()
                               for i in open(self.storeDir+"/Hosts", "r")]
            self.host2index = dict(
                [(host, index) for (index, host) in enumerate(self.index2host)])
        except IOError:
            self.index2host = []
            self.host2index = {}
    ...
    def loadMetrics(self):
        try:
            self.index2metric = [i.rstrip()
                                 for i in open(self.storeDir+"/Metrics", "r")]
            self.metric2index = dict([(metric, index) for (
                index, metric) in enumerate(self.index2metric)])
        except IOError:
            self.index2metric = []
            self.metric2index = {}
    ...
    def savePoint(self, host, metric, timestamp, value):
        ...
        self.curValues.setdefault(
            timestamp/60*60, array.array("c")).extend(self.pointStruct.pack(host, metric, value))

    def flush(self):
        prevDay = 0
        f = None
        if self.metricsUpdated == True:
            open(self.storeDir+"/Metrics",
                 "w").write("\n".join(self.index2metric))
            self.metricsUpdated = False
        if self.hostsUpdated == True:
            open(self.storeDir+"/Hosts", "w").write("\n".join(self.index2host))
            self.hostsUpdated = False
        for (minute, values) in self.curValues.items():
    ...
    def getPoints(self, beginTime, endTime, hosts=None, metrics=None):
        ...
        if hosts:
            ...
            hosts = list(self.host2index.values())
        if metrics:
            try:
            ...
            metrics = list(self.metric2index.values())
        ...
        for day in range(dayStart, endTime+1, self.oneday):
            ...
            while minute <= endTime:
                ...
                for entry in range(size/self.pointStruct.size):
                    ...
                    (host, metric, value) = self.pointStruct.unpack_from(
                        data, entry*self.pointStruct.size)
