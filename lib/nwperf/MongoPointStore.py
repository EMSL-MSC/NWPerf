...
class MongoPointStore(object):
    def __init__(self, mongouri=None):
        ...
        if not "points" in self.db.collection_names():
            self.db.points.ensure_index(
                "time", pymongo.ASCENDING, unique=False, expireAfterSeconds=3600*24*30)
    ...
    def savePoint(self, host, metric, timestamp, value):
        ...
        self.curValues.setdefault(
            timestamp/60*60, {}).setdefault(host, []).append((metric, value))

    def flush(self):
        for timestamp in self.curValues:
            time = datetime.datetime.fromtimestamp(timestamp)
            for host in self.curValues[timestamp]:
                self.db.points.insert(
                    {"time": time, "host": host, "points": self.curValues[timestamp][host]})
        self.curValues = {}
    ...
    def getPoints(self, beginTime, endTime, hosts=None, metrics=None):
        ...
        print("Finding points:", {
              "time": {"$gte": beginTime, "$lte": endTime}, "host": {"$in": hosts}})
        entries = self.db.points.find(
            {"time": {"$gte": beginTime, "$lte": endTime}, "host": {"$in": hosts}})
