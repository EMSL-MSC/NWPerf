            ...
            for i in points:
                try:
                    while hostTimes[i["host"]] + oneMinute < i["time"]:
                        ...
                        for metric in self.graphs.values():
                            metric[i["host"]].append(
                                (hostTimes[i["host"]], None))
                except KeyError:
                    hostTimes[i["host"]] = i["time"]
                hostTimes[i["host"]] = i["time"]
                self.graphs.setdefault(i["pointname"], {}).setdefault(
                    i["host"], []).append((i["time"], i["val"]))
                count += 1

            if (count/len(job["Nodes"])) > 2000:
                print("downsampling", count, len(job["Nodes"]))
                for (metric, nodes) in self.graphs.items():
                    downSampled = {}
