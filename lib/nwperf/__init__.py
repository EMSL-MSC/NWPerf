...
def timeToSeconds(hms):
    ...
    t = datetime.timedelta(days=days, hours=int(
        hms[:2]), minutes=int(hms[3:5]), seconds=int(hms[6:]))
...
def getJobInfo(jid, cluster, nameserver):
    ...
    try:
        ...
        url = srv[-1][0]
        # print srv,url
    except nnslib.NameServerException as msg:
        print("Error:", msg)
        return {}
...
def defaultServerOptionParser():
    ...
    _parser.add_option("-l", "--logfile", dest="logfile", type="string",
                       help="logfile to use. default: /var/log/%s.log" % (prog), default="/var/log/%s.log" % (prog))
    ...
    _parser.add_option("-n", "--nodaemon", dest="nodaemon", action="store_true",
...
if __name__ == "__main__":
    ...
    parser.add_option("-s", "--sleep", dest="sleeptime", type="int",
                      help="The amount of seconds to sleep before exiting", default=5)
