...
class MongoJobStore(JobStore.JobStore):
    ...
    def storeJob(self):
        ...
        endTime = datetime.datetime.strptime(
            self.job["End"], "%Y-%m-%dT%H:%M:%S")
        startTime = datetime.datetime.strptime(
            self.job["Start"], "%Y-%m-%dT%H:%M:%S")
        submitTime = datetime.datetime.strptime(
            self.job["Submit"], "%Y-%m-%dT%H:%M:%S")
        metadata = self.job
        ...
        for graph in self.graphs:
            ...
            graphs.append(
                {"name": graph, "graph": self.db.graphs.insert(self.graphs[graph])})
        print("Appending Graphs", ",".join(
