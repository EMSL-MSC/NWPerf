...
def timeToSeconds(hms):
    ...
    if dayflag != -1:
        ...
        days = 0
    t = datetime.timedelta(days=days, hours=int(
        hms[-8:-6]), minutes=int(hms[-5:-3]), seconds=int(hms[-2:]))
...
def suckJobInfo(jobnumber):
    sp = subprocess.Popen(["/usr/bin/sacct", '-X', '-P',
                          '-o', flds, '-j', jobnumber], stdout=subprocess.PIPE)
    jobs = suckJobs(sp)
    try:
...
def runningJobInfo():
    sp = subprocess.Popen(["/usr/bin/sacct", '-X', '-P',
                          '-s', 'r', '-a', '-o', flds], stdout=subprocess.PIPE)
    jobs = suckJobs(sp)
    return jobs
...
def suckJobs(process):
    ...
    for line in lines:
        line = line.rstrip()
        parts = line.split("|")
        themap = dict(list(zip(headers, parts)))
        try:
...
class JobStatus:
    ...
    def checkUpdate(self):
        ...
        current = list(currentRunningJobs.keys())
        old = list(self.running.keys())
        for jid in old:
            if jid not in current:
                print("job Complete", jid)
                self.completed[jid] = suckJobInfo(
                    jid)  # get the latest info about it
        ...
        for jid in list(self.completed.keys()):
    ...
    def saveState(self):
        try:
            pickle.dump((self.running, self.completed),
                        open(self.statefile, 'w'))
            print("Saved", len(list(self.running.keys())), "running, and", len(
                list(self.completed.keys())), "Completed jobs to statefile")
        except Exception as msg:
            print("error loading state file", msg)

    def loadState(self):
        try:
            self.running, self.completed = pickle.load(
                open(self.statefile, 'r'))
            print("Loaded", len(list(self.running.keys())), "running, and", len(
                list(self.completed.keys())), "Completed jobs from statefile")
        except Exception as msg:
            print("error loading state file", msg)
...
def main(nsurl, cname, myip, statefile, infoport=None, nport=None, eip=None):
    ...
    ns.publishService(cname+".slurm.jobinfo", "tcp://%s:%s" %
                      (eip, port), 600, "req/rep", "JobDictionary")
    ns.publishService(cname+".slurm.jobnotify", "tcp://%s:%s" %
                      (eip, notifyport), 600, "pub/sub", "JobDictionary")
...
if __name__ == "__main__":
    ...
    parser.add_option("-s", "--state-file", help="File to store the current slurm state",
                      dest="statefile", default="/tmp/slurmjobs.state")
    ...
    parser.add_option("-i", "--ip", dest="ip", type="string",
                      help="ip address to bind to. default: %s" % myip, default=myip)
    parser.add_option("-e", "--eip", dest="eip", type="string",
                      help="external ip address to publish. if Not set will default to the same as -i option", default=None)
    parser.add_option("-o", "--notifyport", dest="nport", type="int",
                      help="port to bind to for notifications. default: random", default=None)
    parser.add_option("-f", "--infoport", dest="fport", type="int",
                      help="port to bind to for information. default: random", default=None)
    ...
    main(options.nameserver, options.cluster, options.ip,
         options.statefile, options.fport, options.nport, options.eip)
