        ...
        for stream in streams:
            if stream not in self.streams:
                ...
                if "multipart" in stream:
                    try:
                        self.ns.publishService(
                            stream, "tcp://%s:%s" % (self.ip, port), self.publishTimeout, "pub/sub", "ZmqMultipartPoint")
                    except nnslib.NameServerException as e:
                        if str(e) == "ZmqMultipartPoint is an unknown data type":
                            self.ns.addDataType(
                                "ZmqMultipartPoint", "ZmqMultipart", "host,time,pointname,val,unit", "")
                        else:
                            raise
                else:
                    try:
                        self.ns.publishService(
                            stream, "tcp://%s:%s" % (self.ip, port), self.publishTimeout, "pub/sub", "Point")
                    except nnslib.NameServerException as e:
                        if str(e) == "Point is an unknown data type":
                            self.ns.addDataType(
                                "Point", "JSON", "host,time,pointname,val,unit", "")
                        else:
                            raise
        jsonPoint = '{"host": "%s", "unit": "%s", "val": "%s", "pointname": "%s", "time": "%s"}' % (
            point["host"], point["unit"], point["val"], point["pointname"], point["time"])
...
def main():
    ...
    optparser.add_option("-n", "--nodaemon", dest="nodaemon", action="store_true",
                         help="if true, will not daemonize.  this also disables the logfile.  default: False", default=False)
    optparser.add_option("-p", "--pidfile", dest="pidfile", type="string",
                         help="pidfile to use. default: /var/run/ganglia-zmq.pid", default="/var/run/ganglia-zmq.pid")
    optparser.add_option("-l", "--logfile", dest="logfile", type="string",
                         help="logfile to use. default: /var/log/ganglia-zmq.log", default="/var/log/ganglia-zmq.log")
    optparser.add_option("-x", "--prefix", dest="prefix", type="string",
                         help="prefix to prepend to stream types", default=None)
    optparser.add_option("-i", "--ip", dest="ip", type="string",
