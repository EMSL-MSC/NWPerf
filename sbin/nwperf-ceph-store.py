...
class Metric:
	...
	def flushAndEvict(self):
		evict=None
		for k,v in self.curdata.iteritems():
			if v['dirty']:
				v['dirty']=False
				if TESTMODE: print v['data']
	...
	def run(self):
		...
		while True:
			...
			if len(currentdata) >= WORK_CHUNK_SIZE or (not res and currentdata):
				...
				currentdata=[]
				for o in self.metrics.itervalues():
					o.flushAndEvict()
	...
	def convertWorkToMetics(self,work):
		...
		try:
			for datum in work:
				...
				l.append((data["pointname"],(self.hostlist.index(data["host"]),secs,val)))
				#if data["pointname"] == TEST:
				#	print data
			for k,v in l:
