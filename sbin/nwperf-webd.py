...
class JobArchiveGenerator(multiprocessing.Process):
    ...
    def run(self):
        ...
        while True:
            job = self.q.get()
            print(job)
            startTime = int(time.strftime(
                "%s", time.strptime(job["Start"], "%Y-%m-%dT%H:%M:%S")))
            endTime = int(time.strftime(
                "%s", time.strptime(job["End"], "%Y-%m-%dT%H:%M:%S")))
            points = self.pointStore.getPoints(
                startTime, endTime, job["Nodes"])
    ...
    def run(self):
        ...
        while True:
            ...
            if res:
                ...
                self.pointStore.savePoint(
                    payload["host"], payload["pointname"], payload["time"], payload["val"])
            else:
                count = 0
                self.pointStore.flush()
                try:
                    ...
                    while not connected:
                        try:
                            ...
                            print("Error connecting to %s. Sleeping" %
                                  self.service)
                            time.sleep(1)
                        finally:
                            connected = True
...
def main():
    ...
    children = [JobArchiveGenerator(pointStore, q, jobStore, extraFields={
                                    "cluster": options.cluster}) for i in range(8)]
    for child in children:
        child.start()
    psp = PointStoreProcess(
        pointStore, options.nameserver, options.pointservice)
    psp.start()
    ...
    while True:
        ...
        for i in range(len(children)):
            if not children[i].is_alive():
                children[i].join()
                del(children[i])
                newchild = JobArchiveGenerator(pointStore, q, jobStore, extraFields={
