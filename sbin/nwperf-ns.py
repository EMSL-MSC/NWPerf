...
class Services(object):
    ...
    def addService(self, service, location, timeout, socketType=None, dataType=None):
        ...
        try:
            ...
            self.services[service] = {
                "locations": {location: time.time()+timeout}}
    ...
    def getService(self, service):
        try:
            ...
            return list(self.services[service]["locations"].items())
        except KeyError:
            return None
    ...
    def listServices(self):
        for i in list(self.services.keys()):
    ...
    def expireService(self, service):
        now = time.time()
        self.services[service]["locations"] = dict(
            [(k, v) for (k, v) in self.services[service]["locations"].items() if v > now])
...
def main():
    ...
    parser.add_option("-l", "--logfile", dest="logfile", type="string",
                      help="logfile to use. default: /var/log/nwperf-ns.log", default="/var/log/nwperf.log")
    ...
    parser.add_option("-n", "--nodaemon", dest="nodaemon", action="store_true",
                      help="if flag is set, then this script will not daemonize.  this also disables the logfile.  default: False", default=False)
            ...
            try:
                if msg[1] in ("addService", "replaceService", "removeService", "addDataType", "removeDataType"):
                    ...
                    if rsp != None:
                        ...
                        rsp = ["ERROR", "%s not Found" % {
                            "describeService": "Service", "getDataType": "Data type"}[msg[1]]]
                        if options.nodaemon:
                            print("Response: %s" % rsp)
                        sock.send_multipart(rsp)
